...
1. **Ingest** — `/api/whatsapp/webhook` (new messages) and `/api/whatsapp/backfill`
   ...
   Instagram DM sync (`/api/instagram/sync`, `src/lib/instagram/`) exists but has
   **no UI**: the Meta app is in development mode and the API returns zero
   conversations without App Review, which the owner declined. See LEADS.md.
...
5. **Whose turn** — `estadoContato()` in `leads-shared.ts` reduces each lead to one
   ...
   bringing our own WhatsApp message back, or the manual Instagram marker
